...
impl WebRTCLibrary {
    ...
    pub fn load() -> Result<Self> {
        ...
        let init_service: Symbol<InitServiceFn> = unsafe { lib.get(b"zm_webrtc_init_service")? };

        let shutdown_service: Symbol<ShutdownServiceFn> =
            unsafe { lib.get(b"zm_webrtc_shutdown_service")? };

        let list_camera_streams: Symbol<ListCameraStreamsFn> =
            unsafe { lib.get(b"zm_webrtc_list_camera_streams")? };

        let register_stream: Symbol<RegisterStreamFn> =
            unsafe { lib.get(b"zm_webrtc_register_stream")? };

        let unregister_stream: Symbol<UnregisterStreamFn> =
            unsafe { lib.get(b"zm_webrtc_unregister_stream")? };

        let create_client: Symbol<CreateClientFn> = unsafe { lib.get(b"zm_webrtc_create_client")? };

        let set_offer: Symbol<SetOfferFn> = unsafe { lib.get(b"zm_webrtc_set_offer")? };

        let add_ice_candidate: Symbol<AddIceCandidateFn> =
            unsafe { lib.get(b"zm_webrtc_add_ice_candidate")? };

        let remove_client: Symbol<RemoveClientFn> = unsafe { lib.get(b"zm_webrtc_remove_client")? };

        let get_connection_state: Symbol<GetConnectionStateFn> =
            unsafe { lib.get(b"zm_webrtc_get_connection_state")? };

        let get_stats: Symbol<GetStatsFn> = unsafe { lib.get(b"zm_webrtc_get_stats")? };

        let free_string: Symbol<FreeStringFn> = unsafe { lib.get(b"zm_webrtc_free_string")? };
    ...
    pub fn init_service(&self, camera_id: u32) -> Result<()> {
        let result = unsafe { (self.init_service)(camera_id) };

        if result != 0 {
            return Err(anyhow!(
                "Failed to initialize WebRTC service: error code {}",
                result
            ));
        }
    ...
    pub fn shutdown_service(&self, camera_id: u32) -> Result<()> {
        let result = unsafe { (self.shutdown_service)(camera_id) };

        if result != 0 {
            return Err(anyhow!(
                "Failed to shutdown WebRTC service: error code {}",
                result
            ));
        }
    ...
    pub fn register_stream(&self, camera_id: u32) -> Result<()> {
        let result = unsafe { (self.register_stream)(camera_id) };
    ...
    pub fn unregister_stream(&self, camera_id: u32) -> Result<()> {
        let result = unsafe { (self.unregister_stream)(camera_id) };

        if result != 0 {
            return Err(anyhow!(
                "Failed to unregister stream: error code {}",
                result
            ));
        }
    ...
    pub fn list_camera_streams(&self) -> Result<Vec<CameraStream>> {
        ...
        let streams: Vec<CameraStream> =
            serde_json::from_str(&streams_json).unwrap_or_else(|_| Vec::new());
    ...
    pub fn create_client(&self, client_id: &str, camera_id: u32) -> Result<*mut c_void> {
        ...
        let client_handle = unsafe { (self.create_client)(client_id_cstr.as_ptr(), camera_id) };
    ...
    pub fn set_offer(&self, client_id: &str, offer_sdp: &str) -> Result<String> {
        ...
        let answer_ptr = unsafe { (self.set_offer)(client_id_cstr.as_ptr(), offer_cstr.as_ptr()) };
    ...
    pub fn add_ice_candidate(&self, client_id: &str, candidate: &IceCandidate) -> Result<()> {
        ...
        if result != 0 {
            return Err(anyhow!(
                "Failed to add ICE candidate: error code {}",
                result
            ));
        }
    ...
    pub fn remove_client(&self, client_id: &str) -> Result<()> {
        let client_id_cstr = CString::new(client_id)?;
        let result = unsafe { (self.remove_client)(client_id_cstr.as_ptr()) };
    ...
    pub fn get_connection_state(&self, client_id: &str) -> Result<SessionState> {
        let client_id_cstr = CString::new(client_id)?;
        let state = unsafe { (self.get_connection_state)(client_id_cstr.as_ptr()) };

        if state < 0 {
            return Err(anyhow!(
                "Failed to get connection state: error code {}",
                state
            ));
        }
...
impl WebRTCSession {
    ...
    pub fn new(camera_id: u32) -> Result<Self> {
        ...
        let lib = lib
            .lock()
            .map_err(|_| anyhow!("Failed to lock WebRTC library"))?;
    ...
    pub fn get_stats(&self) -> Result<String> {
        ...
        let (total_frames, total_bytes, clients_connected, clients_disconnected) =
            lib.get_global_stats()?;

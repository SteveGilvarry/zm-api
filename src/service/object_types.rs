...
pub async fn create(state: &AppState, req: CreateObjectTypeRequest) -> AppResult<ObjectTypeResponse> {
    let model = repo::object_types::create(state.db(), &req).await?;
    Ok(ObjectTypeResponse::from(&model))
}

pub async fn update(state: &AppState, id: i32, req: UpdateObjectTypeRequest) -> AppResult<ObjectTypeResponse> {

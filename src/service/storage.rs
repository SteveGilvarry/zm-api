...
pub async fn create(state: &AppState, req: crate::dto::request::CreateStorageRequest) -> AppResult<StorageResponse> {
...
pub async fn update(
    ...
    let updated = repo::storage::update(state.db(), id, name, path, r#type, enabled, scheme, server_id, url).await?;
...
mod tests {
    ...
    async fn test_update_not_found() {
        ...
        let db = MockDatabase::new(DatabaseBackend::MySql)
            ...
            .into_connection();
        let state = AppState::for_test_with_db(db);
        let err = update(&state, 1, Some("n".into()), None, None, None, None, None, None)
            .await
            .err()
            .expect("should err");
        matches!(err, AppError::NotFoundError(_));
    ...
    fn mk(id: u16, name: &str) -> StorageModel {
        ...
        StorageModel { id, path: "/tmp".into(), name: name.into(), r#type: StorageType::Local, url: None, disk_space: None, scheme: Scheme::Deep, server_id: None, do_delete: 0, enabled: 1 }
    }
    ...
    async fn test_update_ok() {
        ...
        let after = StorageModel { name: "new".into(), ..initial.clone() };
        let db = MockDatabase::new(DatabaseBackend::MySql)
            ...
            .into_connection();
        let state = AppState::for_test_with_db(db);
        let out = update(&state, 7, Some("new".into()), None, None, None, None, None, None).await.unwrap();
    ...
    async fn test_create_ok() {
        ...
        let db = MockDatabase::new(DatabaseBackend::MySql)
            ...
            .into_connection();
        let state = AppState::for_test_with_db(db);
        let req = CreateStorageRequest { path: "/tmp".into(), name: "new".into(), r#type: "local".into(), url: None, enabled: 1, scheme: None, server_id: None };

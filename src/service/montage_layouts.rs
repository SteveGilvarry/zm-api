...
pub async fn list_all(state: &AppState, user_id: Option<u32>) -> AppResult<Vec<MontageLayoutResponse>> {
...
pub async fn create(state: &AppState, req: CreateMontageLayoutRequest) -> AppResult<MontageLayoutResponse> {
    let model = repo::montage_layouts::create(state.db(), &req).await?;
    Ok(MontageLayoutResponse::from(&model))
}

pub async fn update(state: &AppState, id: u32, req: UpdateMontageLayoutRequest) -> AppResult<MontageLayoutResponse> {

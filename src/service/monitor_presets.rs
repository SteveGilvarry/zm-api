...
pub async fn list_all(state: &AppState, model_id: Option<u32>) -> AppResult<Vec<MonitorPresetResponse>> {
...
pub async fn create(state: &AppState, req: CreateMonitorPresetRequest) -> AppResult<MonitorPresetResponse> {
    let model = repo::monitor_presets::create(state.db(), &req).await?;
    Ok(MonitorPresetResponse::from(&model))
}

pub async fn update(state: &AppState, id: u32, req: UpdateMonitorPresetRequest) -> AppResult<MonitorPresetResponse> {

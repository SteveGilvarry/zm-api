...
pub async fn create(state: &AppState, req: CreateTriggerX10Request) -> AppResult<TriggerX10Response> {
    let model = repo::triggers_x10::create(state.db(), &req).await?;
    Ok(TriggerX10Response::from(&model))
}

pub async fn update(state: &AppState, monitor_id: u32, req: UpdateTriggerX10Request) -> AppResult<TriggerX10Response> {

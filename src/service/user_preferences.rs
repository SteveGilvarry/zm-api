...
pub async fn list_all(state: &AppState, user_id: Option<u32>) -> AppResult<Vec<UserPreferenceResponse>> {
...
pub async fn create(state: &AppState, req: CreateUserPreferenceRequest) -> AppResult<UserPreferenceResponse> {
    let model = repo::user_preferences::create(state.db(), &req).await?;
    Ok(UserPreferenceResponse::from(&model))
}

pub async fn update(state: &AppState, id: u32, req: UpdateUserPreferenceRequest) -> AppResult<UserPreferenceResponse> {

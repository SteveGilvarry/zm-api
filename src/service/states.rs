...
) -> AppResult<PaginatedResponse<StateResponse>> {
    let (items, total) = repo::states::find_paginated(state.db(), params).await?;
    let responses: Vec<StateResponse> = items.iter().map(StateResponse::from).collect();

...
pub async fn list_all(state: &AppState, manufacturer_id: Option<u32>) -> AppResult<Vec<ModelResponse>> {
...
pub async fn create(state: &AppState, req: crate::dto::request::CreateModelRequest) -> AppResult<ModelResponse> {
    let model = repo::models::create(state.db(), &req).await?;
    Ok(ModelResponse::from(&model))
}

pub async fn update(state: &AppState, id: u32, name: Option<String>, manufacturer_id: Option<i32>) -> AppResult<ModelResponse> {
...
mod tests {
    ...
    async fn test_update_not_found() {
        ...
        let db = MockDatabase::new(DatabaseBackend::MySql)
            ...
            .into_connection();
        let state = AppState::for_test_with_db(db);
        let err = update(&state, 1, Some("x".into()), Some(2)).await.err().expect("should err");
        matches!(err, AppError::NotFoundError(_));
    }
    ...
    fn mk(id: u32, name: &str, man: Option<i32>) -> ModelModel {
        ModelModel { id, name: name.into(), manufacturer_id: man }
    }

    #[tokio::test]
    async fn test_list_all_ok() {
        let db = MockDatabase::new(DatabaseBackend::MySql)
            .append_query_results::<ModelModel, _, _>(vec![vec![mk(1, "m1", None), mk(2, "m2", Some(3))]])
            .into_connection();
        let state = AppState::for_test_with_db(db);
    ...
    async fn test_update_ok() {
        ...
        let db = MockDatabase::new(DatabaseBackend::MySql)
            ...
            .into_connection();
        let state = AppState::for_test_with_db(db);
        let out = update(&state, 3, Some("new".into()), Some(7)).await.unwrap();
    ...
    async fn test_create_ok() {
        ...
        let db = MockDatabase::new(DatabaseBackend::MySql)
            .append_exec_results(vec![MockExecResult { last_insert_id: 55, rows_affected: 1 }])
            .append_query_results::<ModelModel, _, _>(vec![vec![mk(55, "nm", Some(2))]])
            .into_connection();
        let state = AppState::for_test_with_db(db);
        let req = CreateModelRequest { name: "nm".into(), manufacturer_id: Some(2) };

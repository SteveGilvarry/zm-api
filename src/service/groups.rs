...
pub async fn create(state: &AppState, req: crate::dto::request::CreateGroupRequest) -> AppResult<GroupResponse> {
...
mod tests {
    ...
    fn mk_group(id: u32, name: &str) -> GroupModel { GroupModel { id, name: name.into(), parent_id: None } }

    #[tokio::test]
    async fn test_list_all_ok() {
        let db = MockDatabase::new(DatabaseBackend::MySql)
            .append_query_results::<GroupModel, _, _>(vec![vec![mk_group(1, "g1"), mk_group(2, "g2")]])
            .into_connection();
    ...
    async fn test_update_ok_and_not_found() {
        ...
        let db_ok = MockDatabase::new(DatabaseBackend::MySql)
            ...
            .into_connection();
        let state_ok = AppState::for_test_with_db(db_ok);
        assert_eq!(update(&state_ok, 3, Some("new".into())).await.unwrap().name, "new");
        ...
        let db_none = MockDatabase::new(DatabaseBackend::MySql)
            ...
            .into_connection();
        let state_none = AppState::for_test_with_db(db_none);
        assert!(matches!(update(&state_none, 3, Some("x".into())).await.err().unwrap(), AppError::NotFoundError(_)));
    }
    ...
    async fn test_create_and_delete() {
        ...
        let db_create = MockDatabase::new(DatabaseBackend::MySql)
            .append_exec_results(vec![MockExecResult { last_insert_id: 42, rows_affected: 1 }])
            .append_query_results::<GroupModel, _, _>(vec![vec![mk_group(42, "new")]])
            .into_connection();
        let state_create = AppState::for_test_with_db(db_create);
        let req = CreateGroupRequest { name: "new".into(), parent_id: None };

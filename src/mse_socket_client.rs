...
pub enum MseCommand {
    ...
    GetInitSegment { camera_id: u32 },
    #[serde(rename = "get_latest_segment")]
    GetLatestSegment { camera_id: u32 },
    #[serde(rename = "pop_segment")]
    PopSegment { camera_id: u32 },
    #[serde(rename = "try_pop_segment")]
    TryPopSegment { camera_id: u32 },
    #[serde(rename = "get_buffer_stats")]
    GetBufferStats { camera_id: u32 },
    #[serde(rename = "get_buffer_size")]
    GetBufferSize { camera_id: u32 },
    ...
    UnregisterStream { camera_id: u32, stream_id: u32 },
}
...
impl MseSocketClient {
    ...
    pub fn register_stream(
        ...
        match self.send_command(&command)? {
            ...
            MseResponse::Error { message, code } => Err(anyhow::anyhow!(
                "Failed to register stream: {} (code: {:?})",
                message,
                code
            )),
        }
    ...
    pub fn unregister_stream(&self, camera_id: u32, stream_id: u32) -> Result<()> {
        ...
        match self.send_command(&command)? {
            ...
            MseResponse::Error { message, code } => Err(anyhow::anyhow!(
                "Failed to unregister stream: {} (code: {:?})",
                message,
                code
            )),
        }
    ...
    pub fn get_init_segment(&self, camera_id: u32) -> Result<Option<Vec<u8>>> {
        ...
        match self.send_command(&command)? {
            MseResponse::Success {
                ...
                info!(
                    "Got initialization segment for camera {}: {} bytes",
                    camera_id,
                    decoded.len()
                );
                Ok(Some(decoded))
    ...
    pub fn get_latest_segment(&self, camera_id: u32) -> Result<Option<Vec<u8>>> {
        ...
        match self.send_command(&command)? {
            MseResponse::Success {
                ...
                info!(
                    "Got latest segment for camera {}: {} bytes",
                    camera_id,
                    decoded.len()
                );
                Ok(Some(decoded))
            ...
            MseResponse::Error { message, code } => {
                warn!(
                    "Failed to get latest segment: {} (code: {:?})",
                    message, code
                );
                Ok(None)
            }
    ...
    pub fn try_pop_segment(&self, camera_id: u32) -> Result<Option<Vec<u8>>> {
        ...
        match self.send_command(&command)? {
            MseResponse::Success {
                ...
                debug!(
                    "Popped segment for camera {}: {} bytes",
                    camera_id,
                    decoded.len()
                );
                Ok(Some(decoded))
            }
            MseResponse::Success { data: None, .. } => Ok(None),
            MseResponse::Error { message, code } => {
    ...
    pub fn get_buffer_stats(&self, camera_id: u32) -> Result<(usize, u64, u64, u64, u64)> {
        ...
        match self.send_command(&command)? {
            ...
            MseResponse::Error { message, code } => Err(anyhow::anyhow!(
                "Failed to get buffer stats: {} (code: {:?})",
                message,
                code
            )),
        }
    ...
    pub fn get_buffer_size(&self, camera_id: u32) -> Result<usize> {
        ...
        match self.send_command(&command)? {
            ...
            MseResponse::Error { message, code } => Err(anyhow::anyhow!(
                "Failed to get buffer size: {} (code: {:?})",
                message,
                code
            )),
        }

...
use crate::entity::sea_orm_active_enums::{EventCloseMode, DefaultCodec, Importance, Analysing, AnalysisSource, AnalysisImage, Recording, MonitorType, Function, Capturing, Decoding, Rtsp2WebType, Orientation, RecordingSource, OutputContainer};

// Custom validator for state actions
pub fn is_valid_state(value: &str, _ctx: &()) -> garde::Result {
    match value {
        "start" | "stop" | "restart" => Ok(()),
        _ => Err(garde::Error::new("invalid state; must be 'start', 'stop', or 'restart'")),
    }
}
...
pub fn is_valid_alarm_action(value: &str, _ctx: &()) -> garde::Result {
    match value {
        "on" | "off" | "status" => Ok(()),
        _ => Err(garde::Error::new("invalid alarm action; must be 'on', 'off', or 'status'")),
    }
}

...
pub struct MonitorResponse {
    ...
    /// `Camera` or `Speaker` (1.39.30).
    pub device_class: String,
    /// Audio-level alarm detection (1.39.31): on/off, trigger level, alarm score.
...
impl From<monitors::Model> for MonitorResponse {
    fn from(model: monitors::Model) -> Self {
        Self {
            ...
            device_class: enum_str(&model.device_class),
            audio_detection: model.audio_detection,
            audio_threshold: model.audio_threshold,
            audio_alarm_score: model.audio_alarm_score,

...
pub async fn create(db: &DatabaseConnection, req: &crate::dto::request::CreateZonePresetRequest) -> AppResult<ZonePresetModel> {
    ...
    fn parse_zone_type(s: &str) -> crate::entity::sea_orm_active_enums::ZoneType {
        ...
        match s.to_lowercase().as_str() { "inclusive"=>Inclusive, "exclusive"=>Exclusive, "preclusive"=>Preclusive, "inactive"=>Inactive, "privacy"=>Privacy, _=>Active }
    }
    fn parse_units(s: &str) -> crate::entity::sea_orm_active_enums::Units {
        use crate::entity::sea_orm_active_enums::Units::*;
        match s.to_lowercase().as_str() { "percent"=>Percent, _=>Pixels }
    }
    fn parse_check_method(s: &str) -> crate::entity::sea_orm_active_enums::CheckMethod {
        ...
        match s.to_lowercase().as_str() { "filteredpixels"=>FilteredPixels, "blobs"=>Blobs, _=>AlarmedPixels }
    }
    let am = AM {
        id: Default::default(),
...
pub async fn update(
    ...
    fn parse_zone_type(s: &str) -> crate::entity::sea_orm_active_enums::ZoneType { use crate::entity::sea_orm_active_enums::ZoneType::*; match s.to_lowercase().as_str(){"inclusive"=>Inclusive,"exclusive"=>Exclusive,"preclusive"=>Preclusive,"inactive"=>Inactive,"privacy"=>Privacy,_=>Active} }
    fn parse_units(s: &str) -> crate::entity::sea_orm_active_enums::Units { use crate::entity::sea_orm_active_enums::Units::*; match s.to_lowercase().as_str(){"percent"=>Percent,_=>Pixels} }
    fn parse_check_method(s: &str) -> crate::entity::sea_orm_active_enums::CheckMethod { use crate::entity::sea_orm_active_enums::CheckMethod::*; match s.to_lowercase().as_str(){"filteredpixels"=>FilteredPixels,"blobs"=>Blobs,_=>AlarmedPixels} }
    if let Some(v) = name { am.name = Set(v); }
    if let Some(v) = r#type { am.r#type = Set(parse_zone_type(&v)); }
    if let Some(v) = units { am.units = Set(parse_units(&v)); }
    if let Some(v) = check_method { am.check_method = Set(parse_check_method(&v)); }
...
mod tests {
    ...
    async fn test_update_happy_path() {
        ...
        let after = ZonePresetModel { name: "new".into(), r#type: ZoneType::Inclusive, ..initial.clone() };
        ...
        let updated = update(&db, 8, Some("new".into()), Some("inclusive".into()), None, None)
            .await
            .unwrap()
            .unwrap();

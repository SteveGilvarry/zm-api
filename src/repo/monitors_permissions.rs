use sea_orm::*;
use crate::entity::monitors_permissions::{Entity as MonitorsPermissions, Model as MonitorPermissionModel, ActiveModel, Column};
...
pub async fn find_by_id(db: &DatabaseConnection, id: u32) -> AppResult<Option<MonitorPermissionModel>> {
    Ok(MonitorsPermissions::find_by_id(id).one(db).await?)
}

pub async fn find_by_monitor_id(db: &DatabaseConnection, monitor_id: u32) -> AppResult<Vec<MonitorPermissionModel>> {
...
pub async fn find_by_user_id(db: &DatabaseConnection, user_id: u32) -> AppResult<Vec<MonitorPermissionModel>> {
    Ok(MonitorsPermissions::find()
...
pub async fn create(db: &DatabaseConnection, req: &CreateMonitorPermissionRequest) -> AppResult<MonitorPermissionModel> {
...
pub async fn update(db: &DatabaseConnection, id: u32, req: &UpdateMonitorPermissionRequest) -> AppResult<Option<MonitorPermissionModel>> {

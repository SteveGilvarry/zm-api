...
use crate::entity::monitor_presets::{
    ActiveModel, Column, Entity as MonitorPresets, Model as MonitorPresetModel,
};
...
pub async fn find_by_model(
    ...
    model_id: u32,
) -> AppResult<Vec<MonitorPresetModel>> {
...
pub async fn create(
    ...
    let am = ActiveModel {
        ...
        default_rate: Set(req.default_rate.clone().unwrap_or_else(|| "100".to_string())),
        default_scale: Set(req.default_scale.clone().unwrap_or_else(|| "100".to_string())),
    };
...
pub async fn update(
...
) -> AppResult<Option<MonitorPresetModel>> {
    ...
    if let Some(v) = &req.method {
        am.method = Set(Some(v.clone()));
    }
    if let Some(v) = &req.host {
        am.host = Set(Some(v.clone()));
    }
    if let Some(v) = &req.port {
        am.port = Set(Some(v.clone()));
    }
    if let Some(v) = &req.path {
        am.path = Set(Some(v.clone()));
    }
    if let Some(v) = &req.sub_path {
        am.sub_path = Set(Some(v.clone()));
    }
    if let Some(v) = req.width {
        am.width = Set(Some(v));
    }
    if let Some(v) = req.height {
        am.height = Set(Some(v));
    }
    if let Some(v) = req.palette {
        am.palette = Set(Some(v));
    }
    if let Some(v) = req.max_fps {
        am.max_fps = Set(Some(v));
    }
    if let Some(v) = req.controllable {
        am.controllable = Set(v);
    }
    if let Some(v) = &req.control_id {
        am.control_id = Set(Some(v.clone()));
    }
    if let Some(v) = &req.control_device {
        am.control_device = Set(Some(v.clone()));
    }
    if let Some(v) = &req.control_address {
        am.control_address = Set(Some(v.clone()));
    }
    if let Some(v) = &req.default_rate {
        am.default_rate = Set(v.clone());
    }

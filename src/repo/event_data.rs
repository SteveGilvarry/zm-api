...
use crate::entity::event_data::{Entity as EventData, Model as EventDataModel, ActiveModel, Column};
use crate::error::{AppResult, AppError};
use crate::dto::request::event_data::{CreateEventDataRequest, UpdateEventDataRequest};

pub async fn find_all(db: &DatabaseConnection, event_id: Option<u64>) -> AppResult<Vec<EventDataModel>> {
...
pub async fn create(db: &DatabaseConnection, req: &CreateEventDataRequest) -> AppResult<EventDataModel> {
    let timestamp = req.timestamp.as_ref()
...
pub async fn update(db: &DatabaseConnection, id: u64, req: &UpdateEventDataRequest) -> AppResult<Option<EventDataModel>> {
    let Some(model) = find_by_id(db, id).await? else { return Ok(None) };
    let mut am: ActiveModel = model.into();
    
    if let Some(v) = &req.data { am.data = Set(Some(v.clone())); }

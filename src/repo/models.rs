...
pub async fn find_all(db: &DatabaseConnection, manufacturer_id: Option<u32>) -> AppResult<Vec<ModelModel>> {
    let mut query = Models::find();
    if let Some(mid) = manufacturer_id { query = query.filter(crate::entity::models::Column::ManufacturerId.eq(mid as i32)); }
...
pub async fn create(db: &DatabaseConnection, req: &crate::dto::request::CreateModelRequest) -> AppResult<ModelModel> {
    use sea_orm::{ActiveModelTrait, Set};
    use crate::entity::models::ActiveModel as AM;
    let am = AM { id: Default::default(), name: Set(req.name.clone()), manufacturer_id: Set(req.manufacturer_id) };
    Ok(am.insert(db).await?)
}

pub async fn update(db: &DatabaseConnection, id: u32, name: Option<String>, manufacturer_id: Option<i32>) -> AppResult<Option<ModelModel>> {
    use sea_orm::{ActiveModelTrait, Set};
    ...
    if let Some(v) = manufacturer_id { am.manufacturer_id = Set(Some(v)); }
...
mod tests {
    ...
    fn mk(id: u32, name: &str, man: Option<i32>) -> ModelModel {
        ModelModel { id, name: name.to_string(), manufacturer_id: man }
    }
    ...
    async fn test_update_happy_path() {
        ...
        let updated = update(&db, 2, Some("new".into()), Some(5)).await.unwrap().unwrap();

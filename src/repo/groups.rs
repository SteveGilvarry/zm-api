...
pub async fn update(db: &DatabaseConnection, id: u32, name: Option<String>) -> AppResult<Option<GroupModel>> {
...
pub async fn create(db: &DatabaseConnection, req: &crate::dto::request::CreateGroupRequest) -> AppResult<GroupModel> {
    use sea_orm::{ActiveModelTrait, Set};
    use crate::entity::groups::ActiveModel as AM;
    let am = AM { id: Default::default(), name: Set(req.name.clone()), parent_id: Set(req.parent_id) };
...
mod tests {
    ...
    fn mk(id: u32, name: &str) -> GroupModel { GroupModel { id, name: name.into(), parent_id: None } }

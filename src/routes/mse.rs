...
use crate::{handlers::mse, server::state::AppState, util::middleware::auth_middleware};
...
fn mse_routes() -> Router<AppState> {
    Router::new()
        ...
        .route(
            "/streams/{camera_id}/segments/latest.mp4",
            get(mse::get_latest_segment),
        )
        .route(
            "/streams/{camera_id}/segments/{sequence}",
            get(mse::get_segment),
        )
        .route(
            "/streams/{camera_id}/segments/from/{start_sequence}",
            get(mse::get_segments_from),
        )
        // Test endpoints

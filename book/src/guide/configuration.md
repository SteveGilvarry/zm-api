...
2. **`/etc/zm_api/base.toml`** — the packaged defaults layer. Replaced on
   upgrade; don't edit it.
3. **`/etc/zm_api/prod.toml`** — your configuration.
4. **`APP_*` environment variables**, from `/etc/zm_api/zm_api.env`.
...
| `APP_DAEMON__ENABLED` | `false` = passive. Prefer `zm_api-takeover` over setting this by hand |
| `APP_SERVER__PORT` | Default 8080 |
| `RUST_LOG` | `info`, or per-module: `info,zm_api::streaming=debug` |
| `APP_SERVER__MIDDLEWARE__TRUST_PROXY_HEADERS` | `true` only behind a trusted proxy |

`man 5 zm_api.env` documents every variable.
...
JWT signing keys are generated per install into `/var/lib/zm_api/keys` and are
never packaged. To regenerate:

```bash
sudo /usr/share/zm_api/setup-instance.sh   # idempotent; won't overwrite
```

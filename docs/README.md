...
| [ASSISTANT_MCP_PLAN.md](ASSISTANT_MCP_PLAN.md) | Active (design) | One Rust tool registry behind `POST /api/v3/chat/completion` (server-side loop, NDJSON) and an `rmcp` `/mcp` server. Read-only by default; write tools behind config, admin and an HMAC approval token; local-only image policy for cloud models. |
| [ZMNEXT_OBJECT_DETECTION_MAPPING.md](ZMNEXT_OBJECT_DETECTION_MAPPING.md) | Reference (on hold) | How ZoneMinder 1.39.17's `ObjectDetection*` columns and `AI_*` tables would map onto zm-next pipeline JSON. Model path and confidence fit; backend name, NMS, per-class settings don't. |
